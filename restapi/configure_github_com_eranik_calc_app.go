...
func configureAPI(api *operations.GithubComEranikCalcAppAPI) http.Handler {
	...
	api.MathAddHandler = math.AddHandlerFunc(func(params math.AddParams, principal *models.Principal) middleware.Responder {
		return math.NewAddOK().WithPayload(&math.AddOKBody{Res: params.A + params.B})
	})
	api.MathSubtractHandler = math.SubtractHandlerFunc(func(params math.SubtractParams, principal *models.Principal) middleware.Responder {
		result := params.A - params.B
		body := math.SubtractOKBody{Res: result}
		return math.NewSubtractOK().WithPayload(&body)
	})
